...
public class CircleDisplay extends View implements OnGestureListener {
	...
	private void init() {
		...
		mInnerCirclePaint.setColor(ContextCompat.getColor(mContext, R.color.white));
		...
		setValueWidthPercent(35f);
		setDrawText(true);
		setFormatDigits(1);
		setStepSize(0.5f);
		setTextSize(getResources().getDimension(R.dimen.size_text_radio));
	}
	...
	public void setAnimDuration(int durations) {
		mDrawAnimator.setDuration(durations);
	}
